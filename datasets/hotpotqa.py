    ...
    def __init__(self, path: str, tokenizer, max_length: int = 512, doc_stride: int = 128, is_train: bool = True):
        ...
        for ex in raw:
            ...
            for i in range(input_ids.size(0)):
                ...
                # --- Map answer span ---
                start_pos = end_pos = S
                if self.is_train and qa["answer_start"] != -1:
                    start_char = qa["answer_start"]
                    end_char = start_char + len(ans_text)
                    for ti, (sid, (st, ed)) in enumerate(zip(seq_ids, offsets.tolist())):
                        if sid != 1: continue
                        if st <= start_char < ed: start_pos = ti
                        if st < end_char <= ed: end_pos = ti
                
                # --- Map Supporting Facts ---
                sf_mask = torch.zeros(S, dtype=torch.long)
                if sf_char_spans:
                    for (cs, ce) in sf_char_spans:
                        for ti, (sid, (st, ed)) in enumerate(zip(seq_ids, offsets.tolist())):
                            if sid == 1 and not (ed <= cs or st >= ce):
                                sf_mask[ti] = 1
                ...
                if self.is_train:
                    feat["start_positions"] = torch.tensor(start_pos if start_pos != S else 0, dtype=torch.long)
                    feat["end_positions"] = torch.tensor(end_pos if end_pos != S else 0, dtype=torch.long)
                self.features.append(feat)

        # DEBUG CHECK
        total_sf = sum(f["sf_mask"].sum().item() for f in self.features)
        print(f"DEBUG: Total SF tokens loaded: {total_sf}")
...
def collate_train(batch):
    ...
    out = {
        ...
        "context_mask": context_mask,
        "sf_mask": torch.stack([b["sf_mask"] for b in batch]),
